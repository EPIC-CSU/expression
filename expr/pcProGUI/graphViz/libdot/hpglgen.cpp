...
static void
hpgl_begin_job(FILE *ofp, graph_t *g, char **lib, char *user, char *vers, point pages)
{
	Outfile = ofp;
	Pages = pages;
...
static void hpgl_begin_page(point page, double scale, int rot, point offset)
{
    char   buffer[64];
    box    clipWin;
...
static void mkFontCanon (char *old, char *_new)
{
  char      c;
  while ((c = *old++)) {
...
static void hpgl_textline(point p, char *str, int width, double fontsz, double align)
{
    char   buffer[128];
    double old_align;
...
static void hpgl_bezier(point *A, int n, int arrow_at_start, int arrow_at_end)
{
	...
	if (arrow_at_start || arrow_at_end)
		fprintf(stderr,"hpgl_bezier illegal arrow args\n");
    if (isInvis()) return;
