...
static void computeb2bline (int bi, b2bline* lsp);
static void fitlines (int fbi, int lbi);
static void rate (int fli,int lli,int n);
static void fitsplines (int fbi, int lbi, int n, XY ft, XY lt, point *ulpp, point *urpp, point *llpp, point *lrpp);
static int linefits (int fbi, int lbi, int* flip, int* llip);
static int splinefits (int fbi, int lbi, Spline cv);
static void computelinesplit (int fbi, int lbi, int fli, int lli, int* maxlip, XY* maxpp);
static void computesplinesplit (int fbi, int lbi, Spline cv, int* maxlip, XY* maxpp, int* maxcpip);
static void addspline (int fbi, int lbi, Spline cv);
static Spline mkspline (int fli, int lli, XY tang0, XY tang3);
static int intersectsbox (XY pp, XY cp, int bi, XY* ipp);
static int intersectsb2bline (XY fp, XY lp, int li, XY* ipp);
static int onb2bline (int li, XY ip, int usemargin);
static int online (XY fp, XY lp, XY ip);
static void add2bbox (int bi, XY p);
static void parametrize (int fli, int lli);
static XY linepoint (XY fp, XY lp, double t);
static XY splinepoint (Spline cv, double t);
static int ccw (point a, point b, point c);
static XY adjusttangent (point *lpp, point * rpp, point  p);
static XY norm (XY edge);
static XY add (XY p1, XY p2);
static XY sub (XY p1, XY p2);
static double dist (XY p1, XY p2);
static XY scale (XY p, double c);
static double dot (XY p1, XY p2);
static double B1 (double t);
static double B2 (double t);
static double B01 (double t);
static double B23 (double t);
static void mkspaceb (int size);
static void mkspacep (int size);
static void mkspacel (int size);
...
static void fitsplines (int fbi, int lbi, int n, XY ft, XY lt,
	point *ulpp, point *urpp, point *llpp, point *lrpp)
{
    Spline cv;
...
{  /* pp is always in the box */
    ...
    if (cp.x <= bp->LL.x) {
        ...
        if (ipp->y >= bp->LL.y - EPSILON && ipp->y <= bp->UR.y + EPSILON)
            return TRUE;
    }
    if (cp.x >= bp->UR.x) {
        ipp->x = bp->UR.x;
    ...
    if (cp.y <= bp->LL.y) {
        ...
        if (ipp->x >= bp->LL.x - EPSILON && ipp->x <= bp->UR.x + EPSILON)
            return TRUE;
    }
    if (cp.y >= bp->UR.y) {
        ipp->y = bp->UR.y;
