...
//void dot_initialize(int argc, char** argv)
void dot_initialize(FILE* oFile)
{
	//char		*rest,c;
	//int			i,nfiles;
	...
	aginit();
 //NOS
	Output_lang = lang_select("plain");
	Output_file = oFile;		// Set output file.
...
graph_t* 
next_input_graph(FILE* input)
{
	graph_t		*g;
	static FILE	*fp;

	if (fp == NULL) fp = next_input_file(input);
	g = NULL;
	...
	while (fp != NULL) {
...
#else
		if (g = agread(fp)) {
		/* while (g = agread(fp)) {*/ //use single input file
			int n,m;
			n = agnnodes(g);
			m = agnedges(g);
		...
		//fp = next_input_file(); //use single input file
  fp=NULL;
	}
	return g;
}

void dot_init(graph_t* g)
{
	node_t		*n;
	edge_t		*e;
...
void init_ugraph(graph_t* g)
{
	...
	if ((p = agget(g,"fontpath"))) Fontlibpath=p;
	else if ((p = getenv("DOTFONTPATH"))) Fontlibpath = p;
	else Fontlibpath = DEFAULT_FONTPATH;
...
void nodesize(node_t* n, boolean flip)
{
	float		x,y;
	int			ps;
...
void dot_close(graph_t* g)
{
	cleanup_graph(g);
	agclose(g);
}

void dot_terminate(void)
{
	emit_eof();
	//exit(agerrors());
}
