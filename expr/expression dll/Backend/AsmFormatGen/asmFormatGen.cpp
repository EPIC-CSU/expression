...
*******************************************************************/
/*****************************************************************
 *file  :			asmFormatGen.cpp							 *
 *authors :			Atri Mandal									 *
 *last modified :	11/25/2002									 *
 *																 *
 *contents:			ASM Generation								 *
 *comments:-													 *
 *compiled with :	g++											 *
 *for more info :	mandala@ics.uci.edu							 *
 *****************************************************************/

#ifndef _ASMFORMATGEN_CPP_
#define _ASMFORMATGEN_CPP_
...
int printAsmFormat(FILE *fp,NodePtr p)
{
	NodePtr node=NULL;
...
int printOperandsMappingFile(Symtab st, NodePtr node)
{
	FILE *fp; NodePtr n=NULL;int count;

    CharList MyList;
	
	if ((fp = fopen("operandsMappingFile.txt", "w")) == NULL)
	{
		printf("Could not open file: operandsMappingFile.txt\n");
		assert(0);
	}
...
fprintf(fp,"\"\\t.globl\\t<routine>\\n\"\n");
fprintf(fp,"\"\\t.ent\\t<routine>\\n\"\n");
fprintf(fp,"\"<routine>:\\n\"\n");
fprintf(fp,"endTextSectionHeader\n\n");
fprintf(fp,"textSectionFooter\n");
fprintf(fp,"\"\\t.end\\t<routine>\\n\"\n");
...
/*********************Hardcoding ends****************************************/
	...
	//Now select each opcode and dump the format in the operandsmapping file
	fprintf(fp,"\nopndMap\n");
