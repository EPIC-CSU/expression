...
// removes all edges between the node and its MC neighbours
void IGNode::resetMCAdjList()
{
	// delete all adj. MCs from adj. list
	MyLinkedListIterator<IGNode *> *adjIter = _adjNodeList.elementsForward();
	while (adjIter->hasMoreElements()) {
		...
		if (adjNode->isMC()) {
			_adjNodeList.remove(adjNode);
			adjNode->removeAdjNode(this);
		}
	}
	delete adjIter;
}
