...
#include "../../FunctionalSimulator/FuncSim.h"
#include "../IfConversion/ReadPredicationInformation.h"

#include "../IfConversion/PredicationInformation.h"

// Uncomment this out if you'd like to generate code for ST.
//
// #define _ST_SPECIFIC_
//
...
extern int inFinalSpreadInstrs;
...
//set this to 1, to make spread instrs to insert NOPs
extern int insertNOPs;

//prototypes
extern int pipelinedTbz;
extern int stalledTbz;
void compRegClass(Routine *);
...
void print_time(void){
	...
	f=fopen("tbztime.txt","w");
	if(f==NULL){
		printf("Could not open file tbztime.txt...\n");
		_ASSERT(0);
	}
...
extern void compIDToArgsMap(Routine *rout);
...
__declspec( dllexport ) void usedef_live(void){
	...
	while (routineIter->hasMoreElements())
	{
		...
		// Added by SAH, AVS on 08/30/01
		//
		compIDToArgsMap(currRoutine);
...
__declspec( dllexport ) void regAlloc(void){
	...
	while (routineIter->hasMoreElements())
	{
		currRoutine = routineIter->currentElement();
		currRoutine->printName(cout);
		//mark the address operations
#ifdef _ST_SPECIFIC_		
		AddPointerMarkerForAddressOperations(currRoutine);
		AddPointerMarkerForSlots(currRoutine);
#endif
...
__declspec( dllexport ) void trailblazing(void){
	...
	while (routineIter->hasMoreElements())
	{
		...
		if(pipelinedTbz){
			...
			//recompute live dead
		   currRoutine->recursiveRemoveProperty(_LIVEDEAD_);
	...
	//	print_properties();
		...
		//perform transmutations right after traillbazing, before spreadinstrs
		//TransMutation();
...
//do Trailblazing for the whole program
__declspec( dllexport ) void final_spread_instrs(void){
	MyLinkedListIterator<Routine *> *routineIter;
	...
	*/
	...
	//make spread instrs to insert NOPs
	//insertNOPs=1;
	inFinalSpreadInstrs=1;
...
void genC(PRINT_WHEN pW, PRINT_MESG pM)
{
	//GG: 11/01: printC is also a function, not a boolean variable
	/*if (!printC)
		return;*/
...
void recompute_SSA_properties(Routine *rout);
...
int TEMP_START_NUM = 33;
extern int TEMP_CREATION_NUM;

extern void initCallPass(Program *prog);
...
void InstrSel(void){
	...
	// Pointers from call ops to the first instruction in the called routine also needs to be changed.
	//
	setCallPointers(curProgram);
...
void TransMutation(void){
	...
	while (routineIter->hasMoreElements())
	{
		...
		if(doTransMut)
			currRoutine->doPass("TRANSMUTATION");
...
void gccPatch(void)
{
	...
	while (routineIter->hasMoreElements())
	{
		...
		removeGCCSaveRestores(currRoutine);
...
void print_properties(void){
	...
	//pL.push_back(_INSTR_TYPE_);
	pL.push_back(_DUCHAIN_);
	pL.push_back(_UDCHAIN_);
	pL.push_back(_LIVEDEAD_);
...
void print_RAproperties(PropertyNames prop, Routine *rout){
	PropNameList pL;
	
	pL.push_back(prop);
		
	rout->printProperties(cout, pL);
}
...
extern void buildSSA(Routine *rout);

// delete SSA properties and re-compute RSSA.
//
void recompute_SSA_properties(Routine *rout)
{
	if (rout == NULL) return;

	rout->recursiveRemoveProperty(_SSA_);
...
//delete IDToARGS properties
void recompute_IDToARGS_properties(){
	MyLinkedListIterator<Routine *> *rIter = curProgram->routineListIteratorForw();
	...
	if(pbp){
...
#ifdef __EXPRESSION__
		//read in the expression description if required
		if(doTbz && linkEXPRESSION){
			...
			if(dbEnabled==1){
				...
				char *buf[8]={"expression",EXPR_NAME,"-RT","-DB","-FMFILENAME","vliw.fmd","-IFFILENAME","vliw.ifd"};
				...
				char *buf[9]={"expression",EXPR_NAME,"-RT","-DB","-REM","-FMFILENAME","vliw.fmd","-IFFILENAME","vliw.ifd"};
				//first set the thread info, to be able to count the time inside EXPRESSION
				//setTreadInfo(&threadInfo);
				...
				char *buf[7]={"expression",EXPR_NAME,"-RT","-FMFILENAME","vliw.fmd","-IFFILENAME","vliw.ifd"};
				//first set the thread info, to be able to count the time inside EXPRESSION
				//setTreadInfo(&threadInfo);
		...
		if (simNoSSA)
		{
			simCycles = simulateProgram(curProgram, "Before SSA");
		...
		*/
		...
		// Note: May need to check if this is really necessary.
		// For example, use this only if compiling for MIPS architectures using the GCC front-end
		//
		patchForDoubles(curProgram);
		gccPatch();
...
//		remCorrectionOps(curProgram);
		...
		if (simSSA)
		{
			simCycles = simulateProgram(curProgram, "After SSA, Before HTG");
		...
		genC("AFTER_HTG_BEFORE_PRESCH", "HTG_NoPreSch");
		
		if (simHTG)
		{
			simCycles = simulateProgram(curProgram, "After HTG, Before InstrSel");
		...
		if (simISel)
		{
			simCycles = simulateProgram(curProgram, "After InstrSel, Before Tbz");
...
*/
		...
		if (simTbz)
		{
			simCycles = simulateProgram(curProgram, "After Tbz");
		...
		//
		// Also, change the call flow pointers to point to the new first instr in the routine
		//
		remCorrectionOps(curProgram);
		...
		// Patch the propgram routines with register saves & restores so that we can simulate it.
		//
		//addSavesRestores();
		// curProgram->printHTG();

		//do a final spread instrs, after RA
		final_spread_instrs();
		...
		if (simRA)
		{
			simCycles = simulateProgram(curProgram, "After RA");
		...
		if (funcSimRA)
		{
			// simCycles = funcSimulateProgram(curProgram, "After RA");
			printSimResult(simCycles, "After RA");
			printSimResult(numLockStepCycles(), "After RA, LockStep");
		}
		...
		int numInstrs = curProgram->countInstructions();
		int numOpers = curProgram->countOperations();
		cout << "\t Num. Instructions = " << numInstrs << endl;
		cout << "\t Num. Operations = " << numOpers << endl;
		printCountResultToFile(numOpers, numInstrs, "After All Transformations");
		cout << "--------------------------------------------------------" << endl;
		cout << endl;
	}
