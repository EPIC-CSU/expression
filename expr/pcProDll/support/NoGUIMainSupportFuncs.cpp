...
unsigned long simulateRoutine(Routine *curR, char *mesg = "SIMULATING")
{
	clock_count = 0;
...
void printSimSpeedup(unsigned long htgCycleCount,unsigned long tbzCycleCount){
	FILE *fp=fopen("express_performance.txt","a");
...
{
	FILE * fp;
	fp = fopen("express_performance.txt","a");
...
void printCountResultToFile(int numOpers, int numInstrs, char *mesg)
{
	FILE * fp;
	fp = fopen("express_codesize.txt","a");
	
	fprintf(fp,"%s : Num Opers = %9d Num Instrs = %9d\n", curProgram->getName(), numOpers, numInstrs);
...
void remCorrectionOps(Program *prog)
{
	...
	// Also, need to set the control flow for the call ops correctly
	//
	setCallPointers(prog);
}
