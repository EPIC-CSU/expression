...
double decoder_delay(int C, int B, int A, int Ndwl, int Ndbl, int Nspd, 
					 int Ntwl, int Ntbl, int Ntspd, double *Tdecdrive,
...
double decoder_tag_delay(int C, int B, int A, int Ndwl, int Ndbl, int Nspd,
						 int Ntwl, int Ntbl, int Ntspd, double *Tdecdrive, 
						 double *Tdecoder1, double *Tdecoder2, double *outrisetime)
{
        double Ceq,Req,Rwire,rows,tf,nextinputtime,vth;
...
double wordline_delay(int B, int A, int Ndwl, int Nspd, double inrisetime,
					  double *outrisetime)
{
        double Rpdrive;
...
double wordline_tag_delay(int C, int A, int Ntspd, int Ntwl, double inrisetime, 
						  double *outrisetime)
{
        double tf;
...
double bitline_delay(int C, int A, int B, int Ndwl, int Ndbl, int Nspd, 
...
double compare_time(int C, int A, int Ntbl, int Ntspd, double inputtime,
					double *outputtime)
{
...
double mux_driver_delay(int C, int B, int A, int Ndbl, int Nspd, int Ndwl, 
						int Ntbl, int Ntspd, double inputtime, double *outputtime)
{
...
int organizational_parameters_valid(int rows, int cols, int Ndwl, int Ndbl, 
									int Nspd, int Ntwl, int Ntbl, int Ntspd)
{
   ...
   for (Nspd=1;Nspd<=MAXSPD;Nspd=Nspd*2) {
	...
	if (organizational_parameters_valid
		...
		*/
	    ...
	    if (result->cycle_time+1e-11*(result->best_Ndwl+result->best_Ndbl+
...
void output_time_components(int A, time_result_type *result)
{
   ...
   printf(" total data path (with output driver) (ns): %g\n",
	   result->decoder_delay_data/1e-9+result->wordline_delay_data/1e-9+
	   result->bitline_delay_data/1e-9+result->sense_amp_delay_data/1e-9);
   if (A==1)
      printf(" total tag path is dm (ns): %g\n", result->decoder_delay_tag/1e-9+
		result->wordline_delay_tag/1e-9+result->bitline_delay_tag/1e-9+
		result->sense_amp_delay_tag/1e-9+result->compare_part_delay/1e-9+
		result->drive_valid_delay/1e-9);
   else 
...
void output_data(time_result_type *result, time_parameter_type *parameters)
{
   ...
   datapath = result->decoder_delay_data+result->wordline_delay_data+
	   result->bitline_delay_data+result->sense_amp_delay_data+
	   result->data_output_delay;
   if (parameters->associativity == 1) {
         tagpath = result->decoder_delay_tag+result->wordline_delay_tag+
